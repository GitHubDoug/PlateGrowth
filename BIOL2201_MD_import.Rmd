...
SpectraTest <- OD_data %>%
  filter(Wavelength >= 450,
         Wavelength <= 700) %>%
  filter(Filename == "RawData/TestPlate/20230125_1602_FriPlate_20.txt") %>%
  ggplot() +
...
OD_600_long %>% ggplot() +
  geom_line((aes(x = E_hours, y = log(OD_600)))) + 
  facet_grid(rows = vars(Row), cols = vars(Col)) +
  theme_bw()
```
...
saveRDS(OD_600_long, file.path(DataOut, 
paste(Project, "OD_600_long.Rds", sep = "_"), fsep = .Platform$file.sep))
```