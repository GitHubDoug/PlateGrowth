...
ExEm_files <- list.files(path = file.path("StudentData"), pattern = file_id, full.names = TRUE)
...
Export wide format OD_data to OneDrive for Excel Analyses With Just 'HIGH' and 'LOW' plates for each DAY in separate folders, may export data for each DAY simply by re-running script.
...
kable(ExEm_nest |> 
  ...
  unnest(TidiedLog) |>
  select(-statistic) |>
  pivot_wider(names_from = term, values_from = c(estimate:p.value)), digits = 2)
