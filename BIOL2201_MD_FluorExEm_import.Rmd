...
#Files saved do not contain Ex Em nm settings
Ex <- c(440, 520, 620)
Em <- 680
...
#Read in from OneDrive failing; has worked
...
#list of files
ExEm_files <- list.files(path = DataPathMD, pattern = file_id, full.names = TRUE)
...
ExEm_data2 <- ExEm_data |>
  ...
  separate(Filename, into = c("Path1", "Path2", "YYYYMMDD", "HHMM","DAY", "LIGHT", "txt"), sep = "([\\/\\_\\.])", remove = FALSE) |>
...
#smarter way to code this I am sure
 ExEm_data3 <-  ExEm_data2 |>
   group_by(Filename) |>
   mutate(Ex_nm = case_when(row_number() %in% c(1:5) ~ Ex_nm[1],
                            row_number() %in% c(6:10) ~ Ex_nm[2],
                            row_number() %in% c(11:15)  ~ Ex_nm[3]), .after = MeasurePt)
   
  relocate(c("YYYYMMDD", "HHMM","Day", "Light_uE")) |>
