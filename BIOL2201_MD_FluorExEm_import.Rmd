...
MetaData <- readxl::read_xlsx(file.path("~/OneDrive - Mount Allison University/BIOL2201_2024/WI2024Labs/Lab3_MicroGrowth/WellPlateMetaData.xlsx"), sheet = "TEST")
...
ExEm_long <- ExEm_mean |>
  pivot_longer(cols = -c(Filename:Excite_nm, DateTime), names_to = "WELL", values_to = "Fluorescence") |>
...
Merge with MetaData
```{r merge with Meta}
ExEm_Meta <- left_join(ExEm_long, MetaData, by = c("DAY", "LIGHT", "WELL"))
```


```{r test ExEm_Meta plots}
ExEm_Meta |>
  filter(!is.na(SAMPLE)) |>
  ggplot() +
  geom_point(aes(x = E_hours, y = Fluorescence, colour = as.factor(Excite_nm)), show.legend = FALSE) + 
  scale_colour_manual(values = ColourNM) +
  facet_grid(cols = vars(SAMPLE), rows = vars(Excite_nm)) + 
  theme_bw()
