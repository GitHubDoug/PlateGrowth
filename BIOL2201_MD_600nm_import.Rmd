...
DayGroupDayBenchData <- OD_meta %>%
  filter(Group == DayGroup) %>%
  filter(Bench == DayBench) %>%
  select(-c(Row, Col)) %>%
  pivot_wider(names_from = Well, values_from = c(OD_600, logOD_600))
  
  
  
  
write_csv(., file = file.path("ProcessData", paste(DayGroup,"_", Bench, "_", "OD600", ".csv", sep = "")))
```
